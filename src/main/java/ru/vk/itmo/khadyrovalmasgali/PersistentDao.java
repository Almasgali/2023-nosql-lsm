...
public class PersistentDao implements Dao<MemorySegment, Entry<MemorySegment>>, Iterable<Entry<MemorySegment>> {
    ...
    public void compact() throws IOException {
        String index = "0";
        if (Files.exists(config.basePath().resolve(String.format("%s%s", SSTABLE_NAME_PREFIX, index)))) {
            index = "1";
        }
        if (Files.exists(config.basePath().resolve(String.format("%s%s", SSTABLE_NAME_PREFIX, index)))) {
            throw new IllegalStateException("Can't process compact operation.");
        }
        flush(this, Long.parseLong(index));
        for (SSTable table : sstables) {
            table.delete();
        }
        sstables.clear();
        sstables.add(new SSTable(config.basePath(), index, logger, arena));
    }
